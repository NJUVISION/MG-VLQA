...
def tuples_to_wh_questions_with_answers(tuple_output: str):
    questions = []
    entities = []          # 存实体 dict
    globals_ = []

    raw_entities = []      # 只存实体名（先分配 id）
    lines_cache = []       # 把每行缓存下来，后面再扫依赖

    for line in tuple_output.split("\n"):
        line = line.strip()
        ...
        # entity
        if "entity - whole" in line or "entity - part" in line:
            raw_entities.append(parts[0])

        # attribute
        elif "attribute - color" in line and len(parts) == 2:
            obj, color = parts
            questions.append({
        ...
        elif "attribute - state" in line and len(parts) == 2:
            obj, state = parts
            questions.append({
                "question": f"What is the state of the {obj}?",
                "answer": state
            })

        elif "attribute - type" in line and len(parts) == 2:
            obj, type_ = parts
            questions.append({
                "question": f"What type is the {obj}?",
                "answer": type_
            })

        # action
        elif "action" in line and len(parts) >= 2:
            subj = parts[0]
            obj = parts[1]
            act = parts[2] if len(parts) >= 3 else "do"
            act_ing = act + "ing" if not act.endswith("e") else act[:-1] + "ing"
            questions.append({
                "question": f"What is the {subj} {act_ing}?",
                "answer": obj
            })

        # relation
        elif "relation - spatial" in line and len(parts) >= 2:
            obj1 = parts[0]
            obj2 = parts[1]
            rel = parts[2] if len(parts) >= 3 else "related to"
            questions.append({
        ...
        # global
        elif "global" in line and len(parts) >= 1:
            globals_.extend(parts)

    # ---------------------------
    # 1) 给实体分配 id
    # ---------------------------
    for idx, ent_name in enumerate(raw_entities, start=1):
        entities.append({"id": idx, "entity": ent_name})

    name2id = {e["entity"].lower(): e["id"] for e in entities}

    # ---------------------------
    # 2) ✅ 根据非 entity tuple 补实体依赖
    # 规则：只要 line 不是 entity 行
    # 且 parts[0], parts[1] 都是实体
    # 就让 parts[1] 依赖 parts[0]
    # ---------------------------
    for line in lines_cache:
        if "entity - whole" in line or "entity - part" in line:
            continue  # 跳过 entity 行
        ...
        if parent in name2id and child in name2id:
            parent_id = name2id[parent]
            child_id  = name2id[child]

            for e in entities:
                if e["id"] == child_id:
                    # Add parent ID as dependency for child entity
                    if "dependence" not in e:
                        e["dependence"] = []
                    e["dependence"].append(parent_id)
    ...
    if globals_:
        questions.insert(0, {
            "question": "What is the global scene/environment?",
            "answer": globals_
        })
...
def generate_dsg(id2prompts: Dict[str, Dict[str, str]],
    ...
    Args:
        ...
        generate_fn: 调用语言模型的函数
        tuple_train_examples: tuple 生成示例
        dependency_train_examples: dependency 生成示例
        question_train_examples: question 生成示例
        N_parallel_workers: 并行 worker 数
        verbose: 是否打印中间信息

    Returns:
        id2tuple_outputs: tuple 生成结果
        id2question_outputs: 转换后的 WH 问题+答案
    """
    ...
    # =====================================
    # Task 2: Convert Tuples → WH Q&A
    # =====================================
    id2question_outputs = {}
    for id, out in id2tuple_outputs.items():
        tuple_text = out["input"] + "\n" + out.get("output", "")
        wh_qa = tuples_to_wh_questions_with_answers(tuple_text)

        id2question_outputs[id] = {
            "id": id,
            "input": out["input"],
            "output": wh_qa
        }
    ...
    return id2tuple_outputs, id2question_outputs
